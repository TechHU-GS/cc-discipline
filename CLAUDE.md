...
- **Purpose**: Regression matrix for `git-guard.sh`, 156 cases:
...
- **Run awk with `LC_ALL=C` in hooks, and test macOS under a UTF-8 locale.** Under C.UTF-8 or en_US.UTF-8, macOS's awk is still byte-based for `substr()` and `length()`, but a regex match against half a multi-byte character kills it ("towc: multibyte conversion failure"). 2.15.1's session-start died on `## 2026-08-04（三）` and silently fell back to the file's tail. gawk tolerates stray bytes, so only the mac breaks. The ssh login shell on mac-mini has no locale set, so every mac test before 2026-09-25 ran in `C` and hid it: run the matrices there with `LC_ALL=C.UTF-8` as well.
...
- **Run the matrices after touching either guard's matching**: `tests/git-guard-matrix.sh` (156 cases) and `tests/pre-edit-guard-matrix.sh` (21); after touching `session-start.sh`, run `tests/session-start-matrix.sh` (64). Glob, ERE and BRE are different engines; equivalence must be demonstrated, not assumed.
