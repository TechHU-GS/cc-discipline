...
- **Next steps**: (0) **`upgrade` overwrites customized skills** — it wiped this repo's `/self-check` Project-specific Checks (restored by hand from a pre-upgrade copy). That section is explicitly designed for user content, so the installer should merge or preserve it rather than overwriting it. **FIXED in v2.12.2** — manifest-based detection, verified in production on the two 7940 files. (1) **DONE — git-guard commit-message false positive fixed in v2.12.3** (message-argument blanking + tests/git-guard-matrix.sh). (2) **Frozen bucket still undecided.** The Rule Ledger plan failed (see Decision 16); the 2026-08-28 transcript sweep replaced it and settled the *hooks* — git-guard 64 real blocks in 4 weeks on machines where it was dead before 7/30 (keep, no argument); pre-edit-guard's new-script reminder 2,170 firings of near-pure noise (**still to remove**). The frozen *rules* remain undecided because they are always-injected context and leave no mechanical trace — only indirect evidence exists (/think 19 uses, debug-log.md stale in 3 of 4 projects checked). Deleting is irreversible and keeping costs only tokens, so they stay frozen. (3) Security: the npm token pasted in chat 2026-06-05 should be rotated. — DONE this session: `~/.claude/CLAUDE.md` line 44 corrected (it had asked Claude to warn when context is nearly full, which Opus 5 cannot observe: per Anthropic's docs, context awareness exists on Sonnet 5 / Sonnet 4.6 / Sonnet 4.5 / Haiku 4.5, and Opus 5 is not on that list — so the instruction could only be satisfied by guessing, which produced the exact noise the replacement text forbids).
