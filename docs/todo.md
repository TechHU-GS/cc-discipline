...
- [ ] techhu-7940 `techhu-devices` still registers git-guard twice: Git Bash there has no jq, and a jq-less upgrade never touches settings.json (mac-mini's 11/9/9 went to 1 in the 2.14.0 rollout) — revisit: next time work happens in that repository; remove the duplicate entry by hand, or with node.
...
- [ ] Update `~/.claude/CLAUDE.md` by hand on the three machines — `upgrade` never touches it, and this round changed it twice (the subagent line; progress/todo mentions) — revisit: now due, since 2.14.0 is deployed.
