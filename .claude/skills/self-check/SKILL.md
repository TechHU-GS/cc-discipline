...
- `bash tests/git-guard-matrix.sh` — **run after ANY change to git-guard.** 153 cases; a miss there loses uncommitted work and is unrecoverable, so this is the one check that must never be skipped. Also run it against the installed copy: `bash tests/git-guard-matrix.sh .claude/hooks/git-guard.sh`.
...
- `bash tests/session-start-matrix.sh` — run after ANY change to session-start.sh. 61 cases over what it injects from progress.md and todo.md, including CRLF files and a progress.md with no Current Status heading.
